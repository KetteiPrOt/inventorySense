    ...
    <section class="space-y-6 max-w-xl">
        <header>
            ...
            <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
                Asigne o remueva usuarios del rol. El usuario <strong>recibirá o perderá los permisos</strong> de la aplicación asociados al rol.
            </p>
        </header>
                    ...
                    @foreach($role->users as $key => $user)
                        <x-table.tr wire:key="{{$user->id}}">
                            ...
                            <x-table.td class="text-center">
                                ...
                                <x-modal
                                    wire:key="{{$user->id}}"
                                    name="confirm-user{{$user->id}}-deletion" :show="$errors->isNotEmpty()" focusable
                                >
                                    <div class="p-6">                    
                                        <h2 class="text-left text-lg font-medium text-gray-900 dark:text-gray-100">
                                            ¿Seguro que deseas remover el usuario?
                                        </h2>
                        
                                        <p class="mt-1 text-left text-sm text-gray-600 dark:text-gray-400">
                                            Despues de la confirmación el usuario perderá los permisos de la aplicación asociados al rol.
                                        </p>
                                        ...
                                        <div class="mt-6 flex justify-end">
                                            ...
                                            <x-danger-button
                                                x-on:click="
                                                    $dispatch('close')
                                                    setTimeout(() => $wire.removeUser({{$user->id}}, 500));
                                                "
                                                class="ms-3"
                                            >
                                                Quitar
